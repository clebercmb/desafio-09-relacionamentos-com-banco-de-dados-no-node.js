...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const checkInexistentProducts = products
      .map(product => product.id)
      .filter(product => !existentProductsIds.includes(product));

    if (checkInexistentProducts.length) {
      throw new AppError(
        `Could not find product ${JSON.stringify(checkInexistentProducts)}`,
      );
    }
    ...
    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `Quantity not available for ${JSON.stringify(
